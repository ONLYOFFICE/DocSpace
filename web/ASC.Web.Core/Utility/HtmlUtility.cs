...
using ASC.Web.Core.Utility.CodeFormat;
...
namespace ASC.Web.Core.Utility
{
    ...
    public class HtmlUtility
    {
        ...
        public HtmlUtility(IOptionsMonitor<ILog> options, SettingsManager settingsManager, IHttpContextAccessor httpContextAccessor, UserManager userManager, DisplayUserSettingsHelper displayUserSettingsHelper, CommonLinkUtility commonLinkUtility)
        {
        ...
        public string GetFull(string html, bool removeAsccut = true)
        {
            ...
            {
                Log.Error(e);
                return string.Format("{0}<br/>Please contact us: <a href='mailto:{1}'>{1}</a>", e.Message, CompanyWhiteLabelSettings.Instance(SettingsManager).Email);
            }
        }

        private string GetLanguageAttrValue(HtmlNode node)
        {
        ...
        private LangType GetLanguage(HtmlNode node)
        {
        ...
        private void ProcessMaliciousAttributes(HtmlDocument doc)
        {
        ...
        private void ProcessZoomImages(HtmlDocument doc)
        {
        ...
        private void ProcessMaliciousTag(HtmlDocument doc)
        {
            var nodes = doc.DocumentNode.SelectNodes("//*");
            if (nodes == null || nodes.Count == 0)
                return;
        ...
        private void ProcessAscUserTag(HtmlDocument doc)
        {
            ...
            foreach (var node in nodes)
            {
                ...
                styleAttr.Value = "display:inline;";
                node.Attributes.Append(styleAttr);
                node.InnerHtml = UserInfoExtension.RenderProfileLinkBase(UserManager.GetUsers(userId), CommonLinkUtility, DisplayUserSettingsHelper);
            }
        }

        private void ProcessCodeTags(HtmlDocument doc)
        {
        ...
        private void ProcessExternalLinks(HtmlDocument doc)
        {
