...
storiesOf('Components|ToggleContent', module)
   ...
   .add('base', () => {
      ...
      const optionsMultiSelect = options('Children', valuesMultiSelect, ['text'], {
         ...
         switch (item) {
            ...
            case 'toggleContent':
               children.push(<ToggleContent key={i}>
                  {text('text inside another ToogleContent', 'Lorem ipsum dolor sit amet, consectetuer adipiscing elit.')}
               </ToggleContent>);
               break;
            case 'textInput':
               children.push(<TextInput key={i} value="text" onChange={event => alert(event.target.value)} />);
               break;
            default:
               break;
         }
