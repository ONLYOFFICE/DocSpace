(function () {
    ...
    isSupported() {
      if (this.unsupportedBrowsers.hasOwnProperty(this.browser.name)) {
        if (
          ...
          this.browser.userOS === "Linux" &&
          +this.browser.chromeVersion <= 75
        )
          return false;
  ...
  const browserInfo = new BrowserDetector().getBrowserInfo();
  const isDesktop = browserInfo.name === "AscDesktopEditor";
