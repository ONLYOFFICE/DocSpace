...
while [ "$1" != "" ]; do
    case $1 in
        ...
        -? | -h | --help )
            ...
            echo "      -bp, --buildpath           path where generated output is placed"
...
cd ${SRC_PATH}
mkdir -p ${BUILD_PATH}/var/www/services/thumb/client && cp -avrf common/ASC.Thumbnails/* ${BUILD_PATH}/var/www/services/thumb/client && echo -e "Done"

echo "== Publish Build ASC.UrlShortener =="
mkdir -p ${BUILD_PATH}/var/www/services/urlshortener/client && cp -avrf common/ASC.UrlShortener/* ${BUILD_PATH}/var/www/services/urlshortener/client && echo -e "Done"

echo "== Publish ASC.Socket.IO =="
mkdir -p ${BUILD_PATH}/var/www/services/socket/client && cp -avrf common/ASC.Socket.IO/* ${BUILD_PATH}/var/www/services/socket/client && echo -e "Done"
