...
using ASC.Common;
using ASC.Projects.Core.Model.Services.NotifyService;
    ...
    public class NotifyClient
    {
        ...
        public void RegisterSecurityInterceptor()
        {
            ...
            (r, p, scope) =>
            {
                try
                {
                    var projectSecurity = scope.ServiceProvider.GetService<ProjectSecurity>();
                    var factory = scope.ServiceProvider.GetService<EngineFactory>();
                    ...
                    {
                        case "Task":
                            var task = factory.GetTaskEngine().GetByID(entityId, false);

                            if (task == null && projectId != 0)
                            {
                                var project = factory.GetProjectEngine().GetByID(projectId, false);
                        ...
                        case "Message":
                            var discussion = factory.GetMessageEngine().GetByID(entityId, false);
                        ...
                        case "Milestone":
                            var milestone = factory.GetMilestoneEngine().GetByID(entityId, false);
                ...
                {
                    scope.ServiceProvider.GetService<IOptionsMonitor<ILog>>().CurrentValue.Error("Send", ex);
                }
                return false;
            });
        ...
        private TagValue GetReplyToEntityTag(ProjectEntity entity, Comment comment)
        {
