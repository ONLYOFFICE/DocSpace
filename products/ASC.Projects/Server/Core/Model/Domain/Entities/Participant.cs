...
namespace ASC.Projects.Core.Domain
{
    ...
    public class Participant : IComparable
    {
        ...
        public bool IsVisitor { get; internal set; }
        public bool IsFullAdmin { get; internal set; }
    ...
    public class ParticipantHelper
    {
        ...
        public ParticipantHelper(UserManager userManager)
        {
            UserManager = userManager;
        }
        public Participant InitParticipant(Participant participant)
        {
            participant.UserInfo = UserManager.GetUsers(participant.ID);
            participant.IsVisitor = participant.UserInfo.IsVisitor(UserManager);
            participant.IsFullAdmin = participant.UserInfo.IsAdmin(UserManager);
            return participant;
        }
        public ParticipantFull InitParticipant(ParticipantFull participant)
        {
            participant.UserInfo = UserManager.GetUsers(participant.ID);
            participant.IsVisitor = participant.UserInfo.IsVisitor(UserManager);
            participant.IsFullAdmin = participant.UserInfo.IsAdmin(UserManager);
