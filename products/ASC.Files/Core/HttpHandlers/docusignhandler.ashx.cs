...
public class DocuSignHandler
{
    ...
    public DocuSignHandler(RequestDelegate _, IServiceScopeFactory serviceScopeFactory)
    {
...
public class DocuSignHandlerService
{
    ...
    {
        _logger.Info("DocuSign redirect query: " + context.Request.QueryString);
    ...
    {
        _logger.Info("DocuSign webhook: " + context.Request.QueryString);
        try
        {
            var xmldoc = new XmlDocument();
            xmldoc.Load(context.Request.Body);
            _logger.Info("DocuSign webhook outerXml: " + xmldoc.OuterXml);
            ...
            _logger.Info("DocuSign webhook: " + envelopeId + " " + subject + " " + status);
            ...
            switch (status)
            {
                case DocuSignStatus.Completed:
                    ...
                    foreach (XmlNode documentStatus in documentStatuses.ChildNodes)
                    {
                        ...
                        {
                            _logger.Error("DocuSign webhook save document: " + documentStatus.InnerText, ex);
                        }
                    }
                    break;
        ...
        {
            _logger.Error("DocuSign webhook", e);
