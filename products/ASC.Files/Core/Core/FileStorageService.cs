...
public class FileStorageService<T> //: IFileStorageService
{
    ...
    {
        return await GetFolderItemsAsync(parentId, from, count, new[] { filter }, subjectGroup, ssubject, searchText, searchInContent, withSubfolders, orderBy, searchArea, withoutTags, tagNames);
    }
        ...
        SearchArea searchArea = SearchArea.Active, bool withoutTags = false, IEnumerable<string> tagNames = null)
    {
        ...
        try
        {
            (entries, total) = await _entryManager.GetEntriesAsync(parent, from, count, filterTypes, subjectGroup, subjectId, searchText, searchInContent, withSubfolders, orderBy, searchArea, withoutTags, tagNames);
        }
        catch (Exception e)
        {
