...
public class FileStorageService<T> //: IFileStorageService
{
    ...
    public async Task<List<FileOperationResult>> DeleteFileAsync(string action, T fileId, bool ignoreException = false, bool deleteAfter = false, bool immediately = false)
    {
    ...
    public async Task<List<FileOperationResult>> DeleteFolderAsync(string action, T folderId, bool ignoreException = false, bool deleteAfter = false, bool immediately = false)
    {
    ...
    public async Task<List<FileOperationResult>> DeleteItemsAsync(string action, List<T> files, List<T> folders, bool ignoreException = false, bool deleteAfter = false, bool immediately = false)
    {
        return _fileOperationsManager.Delete(_authContext.CurrentAccount.ID, await _tenantManager.GetCurrentTenantAsync(), folders, files, ignoreException, !deleteAfter, immediately, GetHttpHeaders());
    }
    ...
    public async Task DeleteStorageAsync(Guid userId)
    {
        ...
        if (!Equals(folderIdFromMy, 0))
        {
            ...
            await _globalFolderHelper.SetFolderMyAsync(userId);
        }
        ...
        if (!Equals(folderIdFromTrash, 0))
        {
            ...
            await _globalFolderHelper.SetFolderTrashAsync(userId);
        }
    ...
    public async Task<List<FileEntry<T>>> AddToFavoritesAsync(IEnumerable<T> foldersId, IEnumerable<T> filesId)
    {
        ...
        return await InternalAddToFavoritesAsync(foldersId, filesId);
    }
    ...
    public async Task<List<FileEntry<T>>> AddToTemplatesAsync(IEnumerable<T> filesId)
    {
        ...
        return await InternalAddToTemplatesAsync(filesId);
    }
        ...
        foreach (var entry in entries)
        {
            try
            {
                ...
                if (changed)
                {
                    ...
                    {
                        var name = ace.SubjectGroup ? (await _userManager.GetGroupInfoAsync(ace.Id)).Name : (await _userManager.GetUsersAsync(ace.Id)).DisplayUserName(false, _displayUserSettingsHelper);
    ...
    public async Task<List<MentionWrapper>> InternalSharedUsersAsync(T fileId)
    {
        ...
        var users = (await _userManager.GetUsersByGroupAsync(Constants.GroupEveryone.ID))
    ...
    public async Task<List<AceShortWrapper>> SendEditorNotifyAsync(T fileId, MentionMessageWrapper mentionMessage)
    {
            ...
            if (!await _fileSecurity.CanReadAsync(file, recipient.Id))
            {
                ...
                try
                {
                    ...
                    if (showSharingSettings)
                    {
                        ...
                        {
                            await _filesMessageService.SendAsync(file, GetHttpHeaders(), MessageAction.FileUpdatedAccessFor, file.Title, (await _userManager.GetUsersAsync(ace.Id)).DisplayUserName(false, _displayUserSettingsHelper), GetAccessString(ace.Access));
                        }
                    }
        ...
        await _notifyClient.SendEditorMentionsAsync(file, fileLink, recipients, message);
    ...
    public async Task ResendEmailInvitationsAsync(T id, IEnumerable<Guid> usersIds)
    {
        ...
        foreach (var userId in usersIds)
        {
            ...
            var link = await _roomLinkService.GetInvitationLinkAsync(user.Email, share.Access, _authContext.CurrentAccount.ID);
