...
public class RoomLogoManager
{
    ...
    public async Task<Folder<T>> CreateAsync<T>(T id, string tempFile, int x, int y, int width, int height)
    {
        ...
        if (EnableAudit)
        {
            await _filesMessageService.SendAsync(room, Headers, MessageAction.RoomLogoCreated);
        }
    ...
    public async Task<Folder<T>> DeleteAsync<T>(T id)
    {
        ...
        try
        {
            ...
            if (EnableAudit)
            {
                await _filesMessageService.SendAsync(room, Headers, MessageAction.RoomLogoDeleted);
            }
