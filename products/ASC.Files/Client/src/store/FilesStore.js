...
class FilesStore {
        ...
        const request = () =>
            api.files
                ...
                .then(async (data) => {
                    ...
                    this.setFolders(
                        isPrivacyFolder && isMobile ? [] : data.folders
                    );
                    this.setFiles(
                        isPrivacyFolder && isMobile ? [] : data.files
                    );
        ...
        if (isFile) {
            ...
            if (
                !(
                    ...
                    isFavoritesFolder ||
                    (isMyFolder &&
                        (this.filter.filterType || this.filter.search))
                )
            ) {
                fileOptions = this.removeOptions(fileOptions, [
                    "open-location",
        ...
        switch (this.selectedFolderStore.rootFolderType) {
            ...
            case FolderType.SHARE:
                const canCreateInSharedFolder =
                    this.selectedFolderStore.access === 1;
                return (
    ...
    get cbMenuItems() {
        ...
        for (let item of filesItems) {
            if (isDocument(item.fileExst))
                cbMenu.push(FilterType.DocumentsOnly);
                ...
                cbMenu.push(FilterType.ArchiveOnly);
        }
    ...
    getOptions = (selection, externalAccess = false) => {
        ...
        const webFilter = selection.find((x) =>
            canWebFilterEditing(x.fileExst)
        );
        ...
        if (formFillingDocs && !externalAccess)
            AccessOptions.push("FormFilling");
    ...
    createThumbnails = () => {
        ...
        filesList.map((file) => {
            ...
            if (thumbnailStatus === thumbnailStatuses.WAITING)
