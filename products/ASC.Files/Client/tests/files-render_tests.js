...
Scenario('Default settings render test', ({ I }) => {
  ...
  I.seeCurrentUrlEquals('/settings/common');
  ...
  if (deviceType !== 'desktop') I.forceClick('.backdrop-active');
  I.seeCurrentUrlEquals('/settings/admin');
  ...
  if (deviceType !== 'desktop') I.forceClick('.backdrop-active');
  I.seeCurrentUrlEquals('/settings/thirdParty');
...
Scenario('Context menu render test', ({ I }) => {
  ...
  I.openContextMenu();
  I.saveScreenshot(`12.row-many-my-documents-tab.png`);
  if (!isModel) {
    I.seeVisualDiff(`12.row-many-my-documents-tab.png`, {
      tolerance: 1,
      prepareBaseImage: false,
    });
  }
