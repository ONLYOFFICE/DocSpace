...
namespace ASC.Mail.ConnectionTest
{
    internal partial class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            try
            {
                _log = LogManager.GetLogger("ASC.Mail.ConnectionTest");
                ...
                {
                    _log.InfoFormat("[START] ASC.Mail.ConnectionTest -e'{0}' -p'{1}'\r\n", options.Email, options.Password);
        ...
        private static void TestConnectAccountSimple(string email, string password)
        {
            ...
            _log.DebugFormat("Search mailbox settings for domain '{0}'\r\n", domain);
            ...
            foreach (var mbox in testMailboxes)
            {
                ...
                _log.InfoFormat("Attempt #{0} Account: '{1}' Password: '{2}'", ++i, mbox.Account, mbox.Password);

                _log.DebugFormat("{0}Server: '{1}' Port: '{2}' AuthenticationType: '{3}' EncryptionType: '{4}'",
                    mbox.Imap ? "Imap" : "Pop",
                    mbox.Server,
                    mbox.Port,
                    mbox.Authentication,
                ...
                _log.DebugFormat("SmtpServer: '{0}' Port: '{1}' AuthenticationType: '{2}' EncryptionType: '{3}'",
                    mbox.SmtpServer,
                    mbox.SmtpPort,
                ...
                {
                    _log.ErrorFormat("{0} failed: '{1}'", mbox.Imap ? "Imap" : "Pop",
                ...
                {
                    _log.InfoFormat("{0} succeeded", mbox.Imap ? "Imap" : "Pop");
                }

                if (!loginResult.OutgoingSuccess)
                {
                    _log.ErrorFormat("Smtp failed: '{0}'\r\n",
        ...
        public static void TestConnectAccountFromConfig(MailBoxData mbox)
        {
            LoginResult loginResult;

            _log.InfoFormat("Account: '{0}' Password: '{1}'", mbox.Account, mbox.Password);

            _log.InfoFormat("{0}Server: '{1}' Port: '{2}' AuthenticationType: '{3}' EncryptionType: '{4}'",
                mbox.Imap ? "Imap" : "Pop",
                mbox.Server,
                mbox.Port,
                mbox.Authentication,
            ...
            _log.InfoFormat("SmtpServer: '{0}' Port: '{1}' AuthenticationType: '{2}' EncryptionType: '{3}'",
                mbox.SmtpServer,
                mbox.SmtpPort,
                mbox.SmtpAuthentication,
                mbox.SmtpEncryption);
            ...
            if (!loginResult.IngoingSuccess)
            {
                _log.ErrorFormat("{0} failed with Exception: '{1}'", mbox.Imap ? "Imap" : "Pop",
            ...
            if (!loginResult.OutgoingSuccess)
            {
                _log.ErrorFormat("Smtp failed with Exception: '{0}'\r\n", loginResult.OutgoingException.ToString());
            }
            else
            {
                _log.Info("Smtp succeeded\r\n");
