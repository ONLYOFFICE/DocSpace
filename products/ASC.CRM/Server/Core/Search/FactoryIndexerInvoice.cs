...
{
    [Scope]
    public sealed class FactoryIndexerInvoice : FactoryIndexer<DbInvoice>
    {
        public FactoryIndexerInvoice(
       ...
       : base(options, tenantManager, searchSettingsHelper, factoryIndexer, baseIndexer, serviceProvider, cache)
        ...
        {
            var entityDao = DaoFactory.GetInvoiceDao();
            ...
            List<DbInvoice> getData(long start, long stop, DateTime lastIndexed) =>
            ...
            List<int> getIds(DateTime lastIndexed)
            {
                ...
                while (true)
                {
                    var id = GetBaseQuery(lastIndexed)
                                ...
                                .Where(r => r.Id >= start)
                                .OrderBy(x => x.Id)
                                .Skip(BaseIndexer<DbInvoice>.QueryLimit)
                                .Select(x => x.Id)
                                .FirstOrDefault();
