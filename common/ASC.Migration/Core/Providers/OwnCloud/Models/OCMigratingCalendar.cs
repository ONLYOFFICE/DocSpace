...
public class OCMigratingCalendar : MigratingCalendar
{
    ...
    public override int EventsCount => _calendars.Count > 0 && _calendars.Values.First() != null ? _calendars.Values.SelectMany(c => c.SelectMany(x => x.Events)).Count() : _calendars.Count;
    public override string ModuleName => MigrationResource.ModuleNameCalendar;

    private readonly List<OCCalendars> _userCalendars;
