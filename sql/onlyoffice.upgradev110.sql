...
BEGIN
    ...
    UPDATE `tenants_quota` SET `features` = 'domain,audit,controlpanel,healthcheck,ldap,sso,whitelabel,branding,ssbranding,update,support,portals:10000,discencryption' WHERE `tenant` = -1;

    IF EXISTS(SELECT * FROM `tenants_quota` WHERE `tenant` = -1000 AND `features` NOT LIKE '%trial%') THEN
    ...
    IF EXISTS(SELECT * FROM `tenants_quota` WHERE `tenant` = -1000 AND `features` LIKE '%trial%') THEN
        UPDATE `tenants_quota` SET `features` = 'docs,domain,audit,controlpanel,healthcheck,ldap,sso,whitelabel,branding,ssbranding,update,support,portals:10000,discencryption' WHERE tenant = -1000;
    END IF;
