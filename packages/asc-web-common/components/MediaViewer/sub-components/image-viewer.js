...
const StyledViewer = styled(Viewer)`
  ...
  .react-viewer-btn {
    ...
    &:hover {
      background-color: ${(props) => props.theme.mediaViewer.imageViewer.backgroundColor};
    }
  }
  li[data-key='prev'] {
    left: 20px;
...
class ImageViewer extends React.Component {
  ...
  render() {
    const { className, visible, images, inactive, onClose, userAccess } = this.props;
    ...
    const toolbars = userAccess ? customToolbar : customToolbar.filter((x) => x.key !== 'delete');
